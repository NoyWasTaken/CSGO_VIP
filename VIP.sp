...
void Menus_ShowMain(int client)
{
    ...
    menu.Display(client, MENU_TIME_FORVER);
}
...
void Menus_SelectPlayer(int client)
{
    ...
    for(int i = 1; i <= MaxClients; i++)
    {
        if(IsClientInGame(i) && !g_aPlayers[i].isVip())
        {
            ...
            StringToInt(i, szIndex, sizeof(szIndex));
...
{
    int iTaeget = g_iTarget[client];
    int iExpriation = GetTime() + (g_iDuration[client] * DAY_TO_SECONDS);
